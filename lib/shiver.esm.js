...
const isNum$1 = o => typeof o === 'number';
...
function numeric(assert$$1, value, message) {
  const valueIsNum = isNum$1(value);
...
const yesterday = createIsDate(v => v === -1);
const today = createIsDate(v => v === 0);
const tomorrow = createIsDate(v => v === 1);
...
const after = createDateComparison('after', (original, comparison) => original > comparison);
const before = createDateComparison('before', (original, comparison) => original > comparison);
 ...
 * @public
 */
function execute(rules, object, detail = false) {
  assert(
    Array.isArray(rules) &&
